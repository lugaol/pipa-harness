...
## Conflict priority
turn instruction > AGENTS.md > project extension > rules/ > skills/ > vault
...
## Memory
- Session start: read `state/SESSION.md` + `state/PLAN.md` + newest
  `vault/decisions/` note.
- Notes carry `as_of`/`valid_until` frontmatter: check the date before citing;
